...
@section('body_web')
    <div class="row">
        ...
        <div class="col-sm">
            <form action="{{ route('create_order', ['id'=>$id_table]) }}" method="post">
                @csrf
                <div class="shadow">
                    <table class="table table-bordered">
                        <thead class="table-light">
                            <tr>
                                <th colspan="6" class="text-center">Kiểm tra thông tin Bàn {{ $id_table }}</th>
                            </tr>
                            <tr>
                        ...
                        <tbody>
                            @php
                                $total_price = 0;
                            @endphp
                            @for ($q = 0; $q <= count($value_menu['quality']); $q++)
                                @if (!empty($value_menu['quality'][$q]))
                                    @php
                                        $prices = $value_menu['price'][$q] * $value_menu['quality'][$q];
                                        $total_price += $prices;
                                    @endphp
                                    <tr>
                                        <td class="text-center">{{ $value_menu['quality'][$q] }}</td>
                                        <td colspan="2">{{ $value_menu['name'][$q] }}</td>
                                        <td>{{ $value_menu['size'][$q] }}</td>
                                        <td>{{ number_format($value_menu['price'][$q]) }} &#8363; </td>
                                        <td>{{ number_format($prices) }} &#8363; </td>
                                    </tr>
                                    <input type="hidden" value="{{ $value_menu['quality'][$q].'-'.$value_menu['name'][$q].'-'.$value_menu['size'][$q].'-'.$value_menu['price'][$q] }}" name="arraryMenu[]">
                                @endif
                            @endfor
                            <tr class="table-dark">
                                <td colspan="4" class="fw-bolder text-center">Phương thức thanh toán </td>
                                <td colspan="2" class="text-center fw-bolder">Tổng tiền: <span class="text-warning">{{ number_format($total_price) }} &#8363;</span></td>
                            </tr>
                            <script type="text/javascript">
                                function EnableCheckCash(cash){
                                  var num2 = document.getElementById("num2");
                                  num2.disabled = cash.checked ? false : true;
                                  if(!num2.disabled){
                                    num2.focus();
                                  }
                                }
                            ...
                            <tr class="table-dark">
                                <td colspan="6">
                                    <div class="row">
                                        ...
                                        <div class="col-lg">
                                            <div class="form-check">
                                                <input type="radio" class="form-check-input" name="methods" value="3" id="cash" onclick="EnableCheckCash(this)">
                                                <label class="form-check-label" for="cash">Tiền mặt</label>
                                            </div>
                                        </div>
                                        <div class="col-lg">
                                            <div class="form-check">
                                                <input type="radio" class="form-check-input" name="methods" value="4" id="later" checked>
                                                <label class="form-check-label" for="later">Chưa thanh toán</label>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                            ...
                            <tr class="table-dark">
                                <td colspan="3">
                                    <input type="hidden" name="num1" id="num1" value="{{ $total_price }}"/>
                                    <div class="input-group w-75">
                                        <label class="input-group-text" for="num2">Tiền khách</label>
                                        <input type="number" name="num2" id="num2" disabled="disabled" class="form-control"/>
                                    </div>
                                </td>
                                <td colspan="3">
                                    <div class="input-group w-75">
                                        <label class="input-group-text" for="subt">Tiền thừa</label>
                                        <input type="text" name="subt" id="subt" readonly class="form-control"/>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td colspan="1"><a href="{{ route('pic_area_index', ['data'=>Auth::user()->id_area]) }}" class="btn btn-danger w-100">Hủy</a></td>
                                <td colspan="4"></td>
                                <td colspan="1"><button type="submit" class="btn btn-success w-100">Xác nhận</button></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
